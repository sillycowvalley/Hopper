...
//NoMul16:
    ...
    EmitLE()
    {
        ...
        Emit(OpCode.CP_A_H);     
        EmitOffset(OpCode.JR_NC_e, +12); // B >= H -> Exit
        EmitOffset(OpCode.JR_Z_e,  +10); // B == H -> Exit

        Emit(OpCode.LD_A_C);            // LSB
        Emit(OpCode.CP_A_L);      
        EmitOffset(OpCode.JR_NC_e, +6); // C >= L -> Exit
        EmitOffset(OpCode.JR_Z_e,  +4); // C == L -> Exit
...
// Exit:        
    ...
    EmitGE()
    {
        ...
        Emit(OpCode.CP_A_B);     
        EmitOffset(OpCode.JR_NC_e, +12); // H >= B -> Exit
        EmitOffset(OpCode.JR_Z_e,  +10); // H == B -> Exit
        Emit(OpCode.LD_A_L);            // LSB
        Emit(OpCode.CP_A_C);      
        EmitOffset(OpCode.JR_NC_e, +6); // L >= C -> Exit
        EmitOffset(OpCode.JR_Z_e,  +4); // L == C -> Exit
        // HL < BC -> BC > HL
        EmitOffsetByte(OpCode.LD_iIY_d_n, -2, 0); // LSB
// Exit:        
    }
