unit Tokenizer
{
    ...
    uses "/Source/Basic/Source"      // source code lines
    uses "/Source/Basic/Expression"  // recursive descent expression parser
    uses "/Source/Basic/HopperCode"  // code related to Hopper VM byte code
    ...
    Tokenize(string content, ref <string> loopStack, ref <uint> loopLineStack)
    {
        ...
        loop
        {
            ...
            else if ((basicInstruction == Basic.PokeW) || (basicInstruction == Basic.PokeB) || (basicInstruction == Basic.Poke1))
            {
                ...
                else if (basicInstruction == Basic.PokeB)
                {
                    HopperCode.PokeB();
                }
                else
            ...
            else if ((basicInstruction == Basic.Goto) || (basicInstruction == Basic.Gosub))
            {
                ...
                if (noFixup)
                {
                    ...
                    { 
                        Error(21, 'h'); // Internal Error - should never arrive here with <expression> for line number
                        break; 
                    } 
                    if (basicInstruction == Basic.Gosub)
                    {
                        Error(21, 'i'); // Internal Error - should never arrive here for GOSUB
                        break; 
                    }
