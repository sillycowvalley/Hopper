unit Objects
{
    ...
    // Offset 7-8: extended value (MSW for LONG type)
    ...
    const byte symbolOverhead = 11;      // Fixed fields before name (increased from 7)
    ...
    const byte snValue = 5;              // Value/args field offset (2 bytes or 4 bytes)
    
    const byte snLocals = 5;             // same slot as Values, better name for Function arguments|locals
    const byte snOpCodes = 9;            // OpCode stream pointer offset (2 bytes) - NEW!
    const byte snName = 11;              // Name field offset (variable length) - shifted by 2
