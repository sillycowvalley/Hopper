...
### Variable System
- **Tokenization-time symbol resolution**: Variable names converted to integer indices during tokenization
- **Unified symbol table**: Single table with 255 entries (index 0 = undefined/error)
- **Scope encoding**: High bit determines scope - bit 7 clear = global (0-127), bit 7 set = local (128-255)
- **Hardware-assisted access**: 6502 BMI instruction for instant scope selection
- **Auto-creation with defaults**: Classic BASIC behavior - undefined variables auto-created
- **Type inference**: From name suffix (`# 6502 Structured BASIC Specification
...
- **No fragmentation concerns**: Existing allocator handles this well
 ...
 = string) or first assignment
- **Strict runtime typing**: Type mismatch errors prevent silent bugs

**Symbol Table Structure:**
```hopper
struct Symbol {
    byte type;        // Types.Int, Types.String, etc.
    byte nameLen;     // Length of name (for LIST/VARS)
    uint value;       // Data or heap pointer
    // followed by: name characters for LIST output
}
```

**Runtime Variable Access:**
```hopper
// A contains variable index from token
BMI useLocalTable    // High bit set = local scope
// Global access: use lower 7 bits as index
// Local access: mask high bit, use as local table index
```

**Performance Benefits:**
- **Lightning-fast runtime access**: Direct indexing, no string lookups during execution
- **Minimal tokenization overhead**: Hash lookup only when user types (not time-critical)
- **6502-optimized**: Single bit test determines scope instantly
